...
def _filter_handler_kwargs(handler: Callable, kwargs: Dict[str, Any]) -> Dict[str, Any]:
...
def inject_state(param_name: str = "state"):
    ...
    def decorator(handler: Callable):
        async def wrapper(event, **kwargs):
            ...
            return await handler(event, **_filter_handler_kwargs(handler, kwargs))
...
class Router:
    ...
    async def propagate_event(self, update_type: str, event, **kwargs: Any) -> Any:
        ...
        """
        if update_type == "posted" and self.bot_user_id:
